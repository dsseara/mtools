...
options.lw = 1; % line width of graphical elements
options.plw = 2.5; % plot line width
options.fs = 18; % font size
options.font = 'avenir'; % font to use
options.EqualiseY = false;
options.EqualiseX = false;
options.FixLogX = false;
options.FixLogY = false;
options.plot_buffer = 0.01; % how much should you zoom out of the data to show extremes?
options.tick_length = 0.01; 
options.x_minor_ticks = true;
...
for i = 1:length(axesHandles)
	...
	set(axesHandles(i),'FontSize',options.fs,'LineWidth',options.lw, 'fontname', options.font)
...
for i = 1:length(axesHandles)
	...
	set(axesHandles(i),'TickLength',2*tl_temp);
end
...
% Make sure box stays on
for i = 1:length(axesHandles)
	set(axesHandles(i),'box','on');
end
